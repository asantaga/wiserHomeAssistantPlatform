...
- v3.4.20
  - Add an OpenTherm relative modulation level sensor on the HeatHub, preserving zero and fractional percentage readings in both the sensor and existing attributes.
  ...
  - Verify supported OpenTherm temperature writes for 90 seconds, retry a mismatch once, then publish a generic `wiser_opentherm_command_failed` event and persistent warning if the retried value still does not stick. Optional request IDs allow callers to correlate the asynchronous result without Wiser depending on them.
  ...
  - Display physical temperature/humidity sensors with the concise `Wiser Temperature/Humidity Sensor` name while preserving their existing device-registry identifiers.
  - Present smart plugs as outlets and remove duplicate Hub V2 equipment readings while retaining the established `Equipment Power` and `Equipment Total Energy` IDs for the canonical readings.
  - Name room thermostats `Wiser Thermostat`, relying on their Home Assistant area for room context instead of repeating the room name.
