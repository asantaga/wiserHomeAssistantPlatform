# Wiser Home Assistant Integration v3.0.24-Beta2
...
2) Create a new card in lovelace with the following yaml
    ...
    css: |
      ...
      :host {
        --zigbee2mqtt-networkmap-node-color: #1e8be5;
        --zigbee2mqtt-networkmap-node-fill-color: #dcfaf3;
        --zigbee2mqtt-networkmap-node-pinned-color: rgba(190, 56, 93, .6);
        --zigbee2mqtt-networkmap-link-color: #0047a1;
        --zigbee2mqtt-networkmap-hover-color: #be385d;
        --zigbee2mqtt-networkmap-link-selected-color: #1565c0;
        --zigbee2mqtt-networkmap-label-color: rgba(255, 255, 255, 1);
        --zigbee2mqtt-networkmap-arrow-color: rgba(18, 120, 98, 0.7);
        --zigbee2mqtt-networkmap-node-coordinator-color: rgba(255,0,0,0.8);
        --zigbee2mqtt-networkmap-node-router-color: #00b804;
      }
    ```
  ...
  ***NOTES***
  1) I have found it is generally better to add this card to a new tab in panel or sidebar mode so that you can make it nice and big.  The settings above are certainly best suited to panel or sidebar mode tabs.
  2) As this card is designed to work with zigbee2mqtt, we are using css in the card yaml to hide the refresh button.  This is advisable as a) it doesn't do anything and b) if you don't have mqtt installed it will error.
...
- 3.0.24
    ...
    * Add new Wiser Zigbee Network Map sensor to support using the zigbee2mqtt-networkmap card to display zigbee network
