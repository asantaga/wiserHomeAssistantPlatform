...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    def set_smartplug_mode(service):
        ...
        _LOGGER.debug(
            "Set Smartplub called - entity %s mode %s ",
            entity_id,
            smart_plug_mode,
        )
...
class WiserSmartPlug(SwitchEntity):
    ...
    def is_on(self):
        ...
        _LOGGER.debug(
            "Smartplug %s is currently %s", self.smart_plug_id, self._is_on
        )
        return self._is_on
    ...
    async def set_smartplug_mode(self, plug_mode):
        _LOGGER.debug(
            "Setting Smartplug %s Mode to %s ", self.smart_plug_id, plug_mode
        )
