...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for device in data.wiserhub.getDevices():
        wiser_devices.append(WiserDeviceSensor( data,
                                        device.get('id'),
    ...
    wiser_devices.append(WiserSystemOperationModeSensor(data, sensorType = 'Operation Mode'))
...
class WiserDeviceSensor(WiserSensor):
    ...
    async def async_update(self):
        ...
        self._state = self.data.wiserhub.getDevice(self.deviceId). \
            get('DisplayedSignalStrength')
            
    def get_device_name(self):
        """Return the name of the Device"""
        product_type = str(self.data.wiserhub.
        ...
        if product_type == 'Controller':
            ...
            return "Wiser " + product_type + "-" + \
                   self.data.wiserhub.getDeviceRoom(self.deviceId)[
                       'roomName']
        else:
    ...
    def icon(self):
        ...
        try:
            return SIGNAL_STRENGTH_ICONS[self.data.wiserhub
    ...
    def device_state_attributes(self):
        ...
        if device_data.get('ReceptionOfDevice') is not None:
            attrs['device_reception_RSSI'] = (
                device_data.get('ReceptionOfDevice').get('Rssi')
            )
            attrs['device_reception_LQI'] = (
                device_data.get('ReceptionOfDevice').get('Lqi')
            )

        if device_data.get('ReceptionOfController') is not None:
            attrs['controller_reception_RSSI'] = (
                device_data.get('ReceptionOfController').get('Rssi')
            )
            attrs['device_reception_LQI'] = (
                device_data.get('ReceptionOfController').get('Lqi')
            )

        if self.sensor_type in ['RoomStat', 'iTRV', 'SmartPlug'] and device_data.get('BatteryVoltage'):
            attrs['battery_voltage'] = device_data.get('BatteryVoltage')
            attrs['battery_percent'] = (
                int(device_data.get('BatteryVoltage') / BATTERY_FULL * 100)
            )
        ...
        if self.sensor_type == 'RoomStat':
            attrs['humidity'] = (
                self.data.wiserhub.getRoomStatData(self.deviceId).get('MeasuredHumidity')
            )
        return attrs
...
class WiserSystemOperationModeSensor(WiserSensor):
    ...
    def __init__(self, data, device_id=0, sensorType = ''):
        ...
        self.away_temperature = self.data.wiserhub.getSystem().get('AwayModeSetPointLimit')
        _LOGGER.info('{} device init'.format(self.device_name))
