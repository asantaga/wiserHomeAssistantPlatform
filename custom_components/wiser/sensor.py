...
from homeassistant.const import (ATTR_ATTRIBUTION, ATTR_BATTERY_LEVEL,
                                 CONF_ENTITY_NAMESPACE, STATE_UNKNOWN)
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for device in handler.get_hub_data().getDevices():
        wiser_devices.append(WiserDevice(device.get('id'), handler,
...
class WiserDevice(Entity):
    ...
    def icon(self):
        ...
        try:
            return icon_list[self.handler.get_hub_data().
                getDevice(self.deviceId).get('DisplayedSignalStrength')]
    ...
    def name(self):
        ...
        product_type = str(self.handler.get_hub_data().
        ...
        if product_type == 'Controller':
            ...
            return "Wiser " + product_type + "-" + \
                   self.handler.get_hub_data(). \
                       getDeviceRoom(self.deviceId)['roomName']
    ...
    def battery_level(self):
        return self.handler.get_hub_data().getDevice(self.deviceId). \
                   get('BatteryVoltage') / BATTERY_FULL * 100

    @property
    def device_state_attributes(self):
        _LOGGER.debug('State attributes for {} {}'.format(self.deviceId,
                                                          self.sensorType))
        ...
        attrs['displayed_signal_strength'] = device_data. \
        ...
        if device_data.get('ReceptionOfDevice') is not None:
            attrs['device_reception_RSSI'] = device_data. \
                get('ReceptionOfDevice').get('Rssi')
            attrs['device_reception_LQI'] = device_data. \
                get('ReceptionOfDevice').get('Lqi')

        if device_data.get('ReceptionOfController') is not None:
            attrs['controller_reception_RSSI'] = device_data. \
                get('ReceptionOfController').get('Rssi')
            attrs['device_reception_LQI'] = device_data. \
                get('ReceptionOfController').get('Lqi')
        ...
        if self.sensorType == 'RoomStat':
            attrs['humidity'] = self.handler.get_hub_data(). \
                getRoomStatData(self.deviceId).get('MeasuredHumidity')
        return attrs

    @property
    def state(self):
        _LOGGER.debug("**********************************")
        _LOGGER.debug("Wiser Device state requested deviceId : %s",
                      self.deviceId)
        _LOGGER.debug("**********************************")
        return self.handler.get_hub_data().getDevice(self.deviceId). \
            get('DisplayedSignalStrength')
...
class WiserSystemCloudSensor(Entity):
    def __init__(self, handler):
        ...
        self.handler = handler
        self.cloudStatus = self.handler.get_hub_data().getSystem(). \
...
class WiserSystemOperationModeSensor(Entity):
    def __init__(self, handler):
        ...
        self.handler = handler
        self.override_type = self.handler.get_hub_data().getSystem(). \
            get('OverrideType')
        self.away_temperature = self.handler.get_hub_data().getSystem(). \
        ...
        if self.away_temperature:
            try:
                ...
                _LOGGER.debug("Exception" +
                              " : Unexpected value for awayTemperature",
